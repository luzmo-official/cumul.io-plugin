...
class FilterMapper {
  ...
  static compileFilter (name, cumulExpression, values, columnInfo, accumulator) {
    if (!validation.isValidFilter(cumulExpression)) { throw new Error('Received invalid filter', cumulExpression) } else {
      if (columnInfo.type === 'numeric') { this.compileNumericOrDateFilter(name, cumulExpression, values, accumulator, columnInfo.type) } else if (columnInfo.type === 'datetime') {
        if (!util.isEmpty(values)) { values = values.map((val) => new Date(val).getTime()) }

        this.compileNumericOrDateFilter(name, cumulExpression, values, accumulator, columnInfo.type)
      } else if (columnInfo.type === 'hierarchy') { this.compileHierarchyFilter(name, cumulExpression, values, accumulator, columnInfo.dbType) }
    }
  }
  ...
  static compileHierarchyFilter (name, cumulExpression, values, accumulator, dbType) {
    ...
    switch (cumulExpression) {
      ...
      case 'not in': {
        ...
        if (values.includes(null)) { obj.bool.should.push({ bool: { must_not: { exists: { field: name } } } }) }

        if (cumulExpression === 'in') { accumulator.must.push(obj) } else { accumulator.must_not.push(obj) }
  ...
  static compileToRangeFilter (cumulioExpression, value) {
    ...
    if (cumulioExpression === '=') { return [{ expr: '>=', value: range[0] }, { expr: '<=', value: range[1] }] } else { return [{ expr: '<=', value: range[0] }, { expr: '>=', value: range[1] }] }
  }
