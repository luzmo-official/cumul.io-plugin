...
class ElasticSearch {
  ...
  static async queryStreaming(client, query, pushdown) {
    ...
    if (pushdown) {
      ...
      if (utils.isEmpty(query.body.aggs) || utils.isEmpty(query.body.aggs.all) || utils.isEmpty(query.body.aggs.all.composite))
        queryNoGroupBys(client, query, stream);
      else
        queryBatchesPushdown(client, query, stream);
    }
    else
      queryBatchesNonPushdown(client, query, stream);
  ...
  static async getSchema(client, timeout){
    ...
    Object.keys(response)
      .forEach((indexName) => {
        ...
        if (utils.isEmpty(properties) && Object.keys(mappings).length > 0)
          properties = mappings[Object.keys(mappings)[0]].properties;

        if (!utils.isEmpty(properties)){
          indexToOriginalName[indexName.toLowerCase()] = indexName;
          if (!indexesToColumns[indexName.toLowerCase()])
            indexesToColumns[indexName.toLowerCase()] = {};
        ...
        if (!utils.isEmpty(aliases) && !utils.isEmpty(aliases[indexName]) && !utils.isEmpty(aliases[indexName].aliases)) {
          Object.keys(aliases[indexName].aliases).forEach(alias => {
            ...
            if (utils.isEmpty(indexesToColumns[alias.toLowerCase()]))
              indexesToColumns[alias.toLowerCase()] = {};
...
const recursiveAddProperties = (indexName, properties, indexesToColumns, prefix) => {
  return Object.keys(properties).forEach((n) => {
    ...
    if (utils.isEmpty(props.properties)){
      ...
      recursiveAddProperties(indexName, properties[n].properties, indexesToColumns, columnName);
  });
};
...
const getHost = (details) => {
  if (details.oauth2_host)
    return `${details.protocol}//${details.host.toString()}:${details.port}`;
...
const queryNoGroupBys = async (client, query, stream) => {
  ...
  if (utils.isEmpty(queryRes.aggregations))
    queryRes.aggregations = { count: queryRes.hits.total };
  else
    queryRes.aggregations.count = queryRes.hits.total;
...
const queryBatchesPushdown = async (client, query, stream) => {
  ...
  if (!after && queryRes.aggregations.all.buckets.length > 0)
    after = queryRes.aggregations.all.buckets[queryRes.aggregations.all.buckets.length - 1].key;
