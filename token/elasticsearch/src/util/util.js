...
class Utilities {
  ...
  getRecursiveOrFalse (obj, propertyList) {
    if (this.isEmpty(obj)) { return false } else {
  ...
  checkDatetime (metadata, colName) {
    for (let i = 0; i < metadata[0].columns.length; i++) {
      ...
      if (meta.id.toString().toLowerCase() === colName.toString().toLowerCase() && meta.type === 'datetime') { return meta.id.toString() }
    }
    return undefined
  }
  ...
  sortByKey (array, key) {
    return array.sort(function (a, b) {
      const el1 = a[key]
      const el2 = b[key]
      if (el1 < el2) { return -1 } else { return ((el1 > el2) ? 1 : 0) }
    })
  }
  ...
  toOriginalTableName (datasetId, schema) {
    const index = schema.indexToOriginalName[datasetId.toLowerCase()]
    if (index === undefined || index === null) { throw errors.badRequest(`The dataset ${datasetId} could not be found. Are you sure that index (still) exists?`) }
    return index
  }
  ...
  getDatasetSchema (datasetId, schema) {
    if (!schema.indexesToColumns[datasetId.toLowerCase()]) { throw errors.badRequest(`The dataset ${datasetId} could not be found. Are you sure that index still exists?`) }
  ...
  getColumnInfo (datasetId, columnId, schema) {
    ...
    if (!schema) {
      ...
      columnId = datasetId
    } else { datasetSchema = this.getDatasetSchema(datasetId, schema) }
