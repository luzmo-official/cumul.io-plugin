  ...
  app[method](endpoint, (request, resolve, next) => {
    endpointFun(request)
      ...
      .catch(error => {
        ...
        if (error.type && error.type.code) { return resolve.status(error.type.code).json(error) }
  ...
  app[method](endpoint, (request, resolve, next) => {
    endpointFun(request, resolve)
      .then((statusAndStream) => {
        if (statusAndStream.statusCode !== 200) { return resolve.status(statusAndStream.statusCode).json(statusAndStream.body) } else {
      ...
      .catch(error => {
        console.log('error in index', error)
        if (error.type && error.type.code && !error.headers) { return resolve.status(error.type.code).json(error) }
