...
class QueryGenerator {
  ...
  generateQuery( columns, filters, tableId, schema, pushdown ) {
    ...
    const cols = this.generateColumns( columns, schema, pushdown ),
      ...
      where = filterStatements.length > 0 ? `WHERE ${filterStatements.join( ' AND ' )}` : '',
      groupby = cols.groupBy.length > 0 ? `GROUP BY ${cols.groupBy.join( ', ' )}` : '',
      query = `
  ...
  quoteTable( table ) {
    ...
    if ( indexFirstDot === -1 ) {
      ...
      return table.substring( 0, table.indexOf( '.' )) + '.' + `"${table.substring( table.indexOf( '.' ) + 1 )}"`
    }
  }
  ...
  generateColumns( columns, schema, pushdown ) {
    ...
    columns.forEach(( column ) => {
      const columnName = this.getColumnName( column, schema[ column.column_id ], pushdown )
      if ( !pushdown ) {
        selectColumns.push( columnName )
      } else if ( column.aggregation && validation.isValidAggregation( column.aggregation )) {
        selectColumns.push( `${this.wrapInAggregation( columnName, column.aggregation )}` )
      } else {
        selectColumns.push( columnName )
  ...
  getColumnName( column, columnInfo, pushdown ) {
    const originalCaseColName = column.column_id === '*' ? column.column_id : columnInfo.name
    if ( pushdown ) {
      ...
      if ( column.column_id === '*' ) {
        ...
        return this.wrapDateFormatting( this.wrapDateTrunc( column.level, originalCaseColName ))
      } else {
  ...
  parseExpressionValues( values, columnInfo ) {
    if ( values ) {
      if ( columnInfo && columnInfo.type === 'datetime' ) {
        ...
        return values.map(( val ) => this.encodeHierarchyValue( val, columnInfo.db_type ))
      } else if ( columnInfo && columnInfo.type === 'numeric' ) {
        return values.map(( val ) => this.encodeNumericValue( val, columnInfo.db_type ))
      } else {
        console.warn( `Unmapped value in parseExpressionValues ${columnInfo && columnInfo.type}` )
      }
    }
    return values
  }
