...
class Controller {
  ...
  async datasets(request) {
    ...
    // check that x-key and x-token are present (depending on authorization mechanism)
    if (!details.headers['x-key'] || !details.headers['x-token'])
      throw errors.unauthorizedError(
        'Key and Token not included in the request.',
      );

    // First fetch the metadata from your database.
    const datasets = await dbWrapper.getDatasets(details.headers);
    schemaCache.storeDatasets(details, datasets);
    return {
      statusCode: 200,
      body: datasets,
    };
  ...
  async query(request) {
    ...
    const pushdown = details.body.options
      ? details.body.options.pushdown
      : false;
    ...
    if (!schema) {
      schema = await dbWrapper.getDatasets(details.headers);
      schemaCache.storeDatasets(details, schema);
    }
    const schemaForTable = schema.find((table) => table.id === details.body.id);

    const query = await queryGenerator.generateQuery(
      details.body.columns,
      details.body.filters,
      details.body.id,
      schemaForTable,
      pushdown,
    );
